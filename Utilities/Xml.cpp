...
namespace Xml
{
	...
	*/
	...
	XMLNode* XMLNode::DeepClone(XMLDocument* b) const
	{
		XMLNode* clone = this->ShallowClone(b);
		if (!clone) return 0;

		for (const XMLNode* child = this->FirstChild(); child; child = child->NextSibling()) {
			XMLNode* childClone = child->DeepClone(b);
	...
	void XMLDocument::DeepCopy(XMLDocument* b) const
	{
		TIXMLASSERT(b);
		if (b == this) {
		...
		for (const XMLNode* node = this->FirstChild(); node; node = node->NextSibling()) {
			b->InsertEndChild(node->DeepClone(b));
		}
	}
	...
	// (x86_64-w64-mingw32 b) then the check is redundant and gcc and clang emit
